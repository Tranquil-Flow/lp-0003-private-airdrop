...
Before the final checker can pass, proof artifacts must be hash-bound and fresh for the current source. The preferred path is the repository-owned heavy lane:

```bash
bash scripts/run-risc0-heavy-lane.sh
```

That command builds `methods/Cargo.toml` with `cargo risczero`, runs the host fixture with `RISC0_DEV_MODE=0`, writes raw receipt/journal/log output under `target/lp0003-risc0-final` and `submission/raw-logs/`, packages the artifacts, validates the manifest, and extracts proof benchmark evidence.

If the proof is generated by another operator harness, package externally generated `RISC0_DEV_MODE=0` receipt/journal output with:
